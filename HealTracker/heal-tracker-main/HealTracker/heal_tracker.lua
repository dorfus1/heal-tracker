...
local function slashCmd(...)
    ...
    if cmd == 'stop' or cmd == 'quit' or cmd == 'exit' then
        -- Crash-safe pause stop.
        -- IMPORTANT: do NOT call mq.imgui.destroy(), mq.unbind(), logTailerClose(),
        -- or snapshotFight() directly from this slash-command callback. On some
        -- MQ2Lua builds those teardown/re-entry calls can crash the driver inside
        -- mq2lua.dll (vsprintf_s_l), which is the crash shown when using
        -- /healtracker stop. Instead, hide the UI, save only lightweight files,
        -- and park the main loop in a harmless delay. Resume with /healtracker start.
        print('\ag[HealTracker]\ax pausing safely...')
        ...
        print('\ag[HealTracker]\ax paused safely. Use \at/healtracker start\ax to resume.')
        print('\ay[HealTracker]\ax Leave the Lua loaded; avoid /lua stop unless you are camping/relogging.')
        return
    end
...
_G.HT_cleanup = function()
    -- Order matters here. Previously we set shuttingDown=true first, then
    -- destroyed the ImGui callbacks. That left a window where a render
    -- frame could already be mid-Begin/End when destroy ran, which crashes
    -- inside vsprintf_s_l in mq2lua.dll on some MQ builds.
    --
    -- New order:
    --   1. Destroy ImGui callbacks first. After destroy returns, no further
    --      render frames will fire. Any in-progress frame completes its
    --      Begin/End naturally.
    --   2. THEN set shuttingDown and unbind slash commands. By this point
    --      nothing in the render path can run, so the flag flip is safe.
...
_G.HT_boot = function()
    ...
    if not _plugin_active then
        ...
        print('\ag[HealTracker]\ax bindLocalEvents() SKIPPED -- plugin is driving events')
    end
    setupActor()
