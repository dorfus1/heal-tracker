...
local function dispatch_raw(line)
    ...
    if kind == 'heal' then
        -- Heal events are intentionally handled by heal_tracker.lua's
        -- local heal mq.event listeners + Actor broadcast. That path is
        -- more reliable cross-box because each client sees received-heal
        -- text locally. Ignore plugin heal events here to prevent double
        -- counting when both systems see the same heal line.
        return
