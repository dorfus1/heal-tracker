--[[
   ...
   v3.21.2 changes:
     - Added HealTracker logo texture support for main window, mini DPS/heal window, and Last Fight popup.
     - Logo path defaults to C:/E3N Beta/E3NextAndMQNextBinary-main/lua/Heal_tracker_Images/HealTracker.png.
     - v3.21.7: Added extra absolute/relative/backslash texture path fallbacks and power-of-two PNG package.

   v3.21.1 changes:
     - Live DPS mini tracker now uses a true GamParse-style 1-second frozen display snapshot.
     - Parser still reads damage continuously, but visible totals/DPS/rows only replace once per second instead of counting upward every frame.
   ...
   v3.20.10 changes:
     - History tab Player Compare now displays even on older archived fights that do not have damage-type data.
     - Older saved fights can now compare Total Damage, DPS, Hits, Max Hit, Spell Casts, spell lists, and observed burn timelines from History.
...
-- =============================================================================
-- HealTracker logo textures
-- =============================================================================
-- Put your logo in this folder:
--   C:\E3N Beta\E3NextAndMQNextBinary-main\lua\Heal_tracker_Images\
-- Preferred file name:
--   HealTracker.png
-- This loader also tries common fallback names/extensions in case Windows is
-- hiding the real extension. Everything is stored on _G so this large Lua file
-- does not risk hitting Lua's 200-local main-chunk limit.
-- The logo image lives alongside the script in heal_tracker_images/.
-- We resolve its path at runtime so this script works on ANY MQ install
-- (E3N, vanilla MQNext, RedGuides, custom paths) without the user having
-- to edit anything. Strategy: probe debug.getinfo to find the script's
-- own directory, then mq.luaDir, then a list of common install layouts.
-- All helpers live in a do-end block so they don't consume main-chunk
-- local slots (Lua hard-caps the main function at 200 locals).
do
    local function _ht_dirname(p)
    ...
    local function _ht_normalize_dir(d)
    ...
    -- Path of THIS Lua file via debug.getinfo (most reliable across MQ builds).
    local scriptDir
    local info = debug and debug.getinfo and debug.getinfo(1, 'S')
    if info and info.source then
        local src = info.source
        if src:sub(1, 1) == '@' then src = src:sub(2) end
        local d = _ht_dirname(src)
        if d and d ~= '' then scriptDir = _ht_normalize_dir(d) end
    end
    -- Fallback: MQ's published lua directory.
    if not scriptDir and mq and mq.luaDir then
        scriptDir = _ht_normalize_dir(mq.luaDir)
    end

    _G.HT_SCRIPT_DIR = scriptDir
    _G._ht_normalize_dir = _ht_normalize_dir -- reused by HT_LoadLogoTextures
end

if not _G.HT_LOGO_PATH then
    if _G.HT_SCRIPT_DIR then
        _G.HT_LOGO_PATH = _G.HT_SCRIPT_DIR .. 'heal_tracker_images/HealTracker.png'
    else
        _G.HT_LOGO_PATH = 'heal_tracker_images/HealTracker.png'
    end
end
-- Reset these on every /lua reload so stale _G flags from a previous run
-- cannot make the script think the logo loaded when the texture is actually nil.
_G.HT_LOGO_MAIN = nil
_G.HT_LOGO_MINI = nil
_G.HT_LOGO_TINY = nil
...
-- Reset draw-side diagnostics on every reload so the new run's first error
-- prints fresh instead of being silenced by a stale "already warned" flag.
_G.HT_LOGO_DRAW_WARNED = false
_G.HT_LOGO_DRAW_DEBUGGED = false
_G.HT_LOGO_DRAW_ERR_PRINTED = false

function HT_LoadLogoTextures(forceRetry)
    if _G.HT_LOGO_MAIN and not forceRetry then return true end

    _G.HT_LOGO_LOAD_ATTEMPTED = true
    _G.HT_LOGO_LOAD_FAILED = false
    _G.HT_LOGO_LAST_ERROR = ''

    local norm = _G._ht_normalize_dir or function(d)
        if not d or d == '' then return nil end
        d = tostring(d):gsub('\\', '/')
        if d:sub(-1) ~= '/' then d = d .. '/' end
        return d
    ...
    -- Build the base-dir list dynamically.
    local baseDirs = {}
    local function addBase(d)
        if not d or d == '' then return end
        d = norm(d)
        if not d then return end
        baseDirs[#baseDirs+1] = d
        -- Also add a backslash variant for MQ builds that prefer Windows-style paths.
        baseDirs[#baseDirs+1] = (d:gsub('/', '\\'))
    end

    -- 1) Script's own directory + heal_tracker_images/ (the canonical install layout).
    if _G.HT_SCRIPT_DIR then
        addBase(_G.HT_SCRIPT_DIR .. 'heal_tracker_images')
        addBase(_G.HT_SCRIPT_DIR) -- image sitting directly next to the lua
    end
    -- 2) MQ's lua dir (if exposed and different from script dir).
    if mq and mq.luaDir then
        addBase(norm(mq.luaDir) .. 'heal_tracker_images')
    end
    -- 3) Generic relative paths (work when MQ's CWD is the lua dir).
    addBase('heal_tracker_images')
    addBase('./heal_tracker_images')
    addBase('lua/heal_tracker_images')
    addBase('./lua/heal_tracker_images')
    -- 4) Mixed-case folder names (legacy / Windows-tolerant fallbacks).
    addBase('Heal_tracker_Images')
    addBase('./Heal_tracker_Images')
    addBase('lua/Heal_tracker_Images')
    ...
    local paths = { tostring(_G.HT_LOGO_PATH or '') }
    for _, baseDirTry in ipairs(baseDirs) do
        for _, nameTry in ipairs(names) do
            table.insert(paths, baseDirTry .. nameTry)
        end
    end
    local baseDir = baseDirs[1]

    local function tryCreateTexture(p)
        if not p or p == '' then return nil end

        -- Most MacroQuest Lua builds expose texture loading on mq.CreateTexture.
        -- Try it first because some builds also expose ImGui.CreateTexture but
        -- do not return a value that ImGui.Image can draw correctly.
        if mq and mq.CreateTexture then
            local ok, tex = pcall(mq.CreateTexture, p)
            if ok and tex then return tex end
            if not ok then _G.HT_LOGO_LAST_ERROR = tostring(tex or '') end
        end

        if ImGui and ImGui.CreateTexture then
            local ok, tex = pcall(ImGui.CreateTexture, p)
            if ok and tex then return tex end
            if not ok then _G.HT_LOGO_LAST_ERROR = tostring(tex or '') end
        end
    ...
    for _, path in ipairs(paths) do
        local tex = tryCreateTexture(path)
        if tex then
            _G.HT_LOGO_MAIN = tex
            _G.HT_LOGO_MINI = tex
            _G.HT_LOGO_TINY = tex
            _G.HT_LOGO_LOAD_FAILED = false
            _G.HT_LOGO_LOADED_PATH = path
            print('\ag[HealTracker]\ax Logo loaded: ' .. path)
            return true
        end
    end

    _G.HT_LOGO_LOAD_FAILED = true
    print('\ay[HealTracker]\ax Logo image was not loaded. Put the included HealTracker.png here: ' .. baseDir)
    if tostring(_G.HT_LOGO_LAST_ERROR or '') ~= '' then
        print('\ay[HealTracker]\ax Logo load detail: ' .. tostring(_G.HT_LOGO_LAST_ERROR))
    end
    return false
end

-- Resolve a usable image handle from whatever the texture loader returned.
-- Different MQ builds return:
--   * a userdata bindable directly by ImGui.Image
--   * a userdata supporting :GetTextureID() returning a number/userdata
--   * a table with a .TextureID / .ID field
-- userdata methods aren't enumerable via pairs(), so we have to probe by name.
function HT_ResolveTextureID(tex)
    if not tex then return nil end
    -- Probe userdata for common ID-fetch methods.
    local probeMethods = { 'GetTextureID', 'GetID', 'TextureID', 'ID', 'Handle', 'GetHandle' }
    for _, mname in ipairs(probeMethods) do
        local okGet, val = pcall(function()
            local m = tex[mname]
            if type(m) == 'function' then
                return tex[mname](tex) -- m:method(self) style
            end
            return m
        end)
        if okGet and val ~= nil and val ~= tex then
            return val
        end
    end
    return tex
...
-- Probe what's actually exposed in this MQ build. Prints once per reload so
-- we can see what overloads/types are present.
function HT_DescribeImGuiAndTexture(tex)
    local function describeAny(v)
        local t = type(v)
        if t == 'table' then
            local keys = {}
    ...
    -- Test which member names exist on the userdata via pcall.
    local userdataMembers = {}
    if type(tex) == 'userdata' or type(tex) == 'table' then
        local probes = {
            'GetTextureID', 'GetID', 'TextureID', 'ID', 'Handle',
            'GetHandle', 'GetSize', 'Draw', 'SetTextureMemory', 'GetTexture',
        }
        for _, name in ipairs(probes) do
            local ok, val = pcall(function() return tex[name] end)
            if ok and val ~= nil then
                userdataMembers[#userdataMembers+1] = name .. '(' .. type(val) .. ')'
            end
        end
    end

    -- Check which texture/draw functions are exposed on mq and ImGui.
    local mqFuncs = {}
    if mq then
        for _, name in ipairs({'CreateTexture', 'LoadTexture', 'LoadTextureAnimation',
                               'DrawTextureAnim', 'CreateTextureAnimation', 'FindTexturePackage'}) do
            if mq[name] ~= nil then mqFuncs[#mqFuncs+1] = name end
        end
    end
    local imguiFuncs = {}
    if ImGui then
        for _, name in ipairs({'Image', 'ImageButton', 'DrawTextureAnimation',
                               'AddImage', 'GetWindowDrawList'}) do
            if ImGui[name] ~= nil then imguiFuncs[#imguiFuncs+1] = name end
        end
    end

    print(string.format('\ag[HealTracker]\ax Logo probe -- tex type: %s', describeAny(tex)))
    print(string.format('\ag[HealTracker]\ax Logo probe -- tex members: %s',
        #userdataMembers > 0 and table.concat(userdataMembers, ', ') or '(none found)'))
    print(string.format('\ag[HealTracker]\ax Logo probe -- mq funcs: %s', table.concat(mqFuncs, ', ')))
    print(string.format('\ag[HealTracker]\ax Logo probe -- ImGui funcs: %s', table.concat(imguiFuncs, ', ')))
    print(string.format('\ag[HealTracker]\ax Logo probe -- ImVec2 exists: %s, ImGui.ImVec2 exists: %s',
        tostring(ImVec2 ~= nil), tostring(ImGui and ImGui.ImVec2 ~= nil)))
end

-- Draw a previously-loaded texture. Returns true ONLY if a draw call actually
-- succeeded. The previous version used pcall around a closure that could
-- no-op silently and still report success.
function HT_DrawLogoImage(tex, w, h)
    if not tex then return false end

    -- One-shot probe of what's actually available on this MQ build.
    if not _G.HT_LOGO_DRAW_DEBUGGED then
        _G.HT_LOGO_DRAW_DEBUGGED = true
        pcall(HT_DescribeImGuiAndTexture, tex)
    end

    local id = HT_ResolveTextureID(tex)

    local uv0a, uv0b = 0, 0
    local uv1a, uv1b = 1, 1
    local tintR, tintG, tintB, tintA = 1, 1, 1, 1
    local bordR, bordG, bordB, bordA = 0, 0, 0, 0

    local attempts = {}

    -- mq.DrawTextureAnim path -- modern MQ builds expose this for textures
    -- returned by mq.CreateTexture. It typically takes (texture, width, height).
    if mq and mq.DrawTextureAnim then
        attempts[#attempts+1] = function() mq.DrawTextureAnim(tex, w, h); return true end
        if ImVec2 then
            attempts[#attempts+1] = function() mq.DrawTextureAnim(tex, ImVec2(w, h)); return true end
        end
    end
    -- ImGui.DrawTextureAnimation -- some builds expose it on the ImGui table.
    if ImGui and ImGui.DrawTextureAnimation then
        attempts[#attempts+1] = function() ImGui.DrawTextureAnimation(tex, w, h); return true end
        if ImVec2 then
            attempts[#attempts+1] = function() ImGui.DrawTextureAnimation(tex, ImVec2(w, h)); return true end
        end
    end

    if ImGui and ImGui.Image then
        -- Full 6-arg form with ImVec2/ImVec4
        if ImVec2 and ImVec4 then
            attempts[#attempts+1] = function()
                ImGui.Image(id, ImVec2(w, h), ImVec2(uv0a, uv0b), ImVec2(uv1a, uv1b),
                    ImVec4(tintR, tintG, tintB, tintA), ImVec4(bordR, bordG, bordB, bordA))
                return true
            end
            attempts[#attempts+1] = function()
                ImGui.Image(id, ImVec2(w, h), ImVec2(uv0a, uv0b), ImVec2(uv1a, uv1b))
            ...
            -- And with the original tex (in case sol2 needs the original userdata, not the resolved id)
            attempts[#attempts+1] = function()
                ImGui.Image(tex, ImVec2(w, h), ImVec2(uv0a, uv0b), ImVec2(uv1a, uv1b),
                    ImVec4(tintR, tintG, tintB, tintA), ImVec4(bordR, bordG, bordB, bordA))
                return true
            end
            attempts[#attempts+1] = function()
        ...
        if ImGui.ImVec2 and ImGui.ImVec4 then
            attempts[#attempts+1] = function()
                ImGui.Image(id, ImGui.ImVec2(w, h), ImGui.ImVec2(uv0a, uv0b), ImGui.ImVec2(uv1a, uv1b),
                    ImGui.ImVec4(tintR, tintG, tintB, tintA), ImGui.ImVec4(bordR, bordG, bordB, bordA))
                return true
            end
            attempts[#attempts+1] = function()
                ImGui.Image(id, ImGui.ImVec2(w, h), ImGui.ImVec2(uv0a, uv0b), ImGui.ImVec2(uv1a, uv1b))
            ...
            attempts[#attempts+1] = function()
                ImGui.Image(id, ImGui.ImVec2(w, h))
                return true
            end
        end

        -- Plain-number expanded forms
        attempts[#attempts+1] = function()
            ImGui.Image(id, w, h, uv0a, uv0b, uv1a, uv1b)
            return true
        end
        attempts[#attempts+1] = function()
            ImGui.Image(id, w, h, uv0a, uv0b, uv1a, uv1b, tintR, tintG, tintB, tintA)
            return true
        end
        attempts[#attempts+1] = function() ImGui.Image(id, w, h); return true end
        attempts[#attempts+1] = function() ImGui.Image(tex, w, h); return true end
    end
    ...
    if not _G.HT_LOGO_DRAW_ERR_PRINTED then
        _G.HT_LOGO_DRAW_ERR_PRINTED = true
        print('\ay[HealTracker]\ax ImGui.Image rejected every signature. Last error: ' .. tostring(lastErr))
    end
    return false
end
...
-- =============================================================================
-- heal_tracker_fastparse.lua must be in the same directory as this file.
-- If the require fails for any reason, _G._ht_fastparse stays nil and the
-- original (slow) functions defined below run unchanged. We stash this on
-- _G instead of using a top-level `local` to avoid pushing the main chunk's
-- local count over Lua's strict limit (the script already runs near it).
do
    local ok, mod = pcall(require, 'heal_tracker_fastparse')
    if ok then _G._ht_fastparse = mod end
end

-- Diagnostic profiler (optional). If heal_tracker_profile.lua is present,
-- it will measure where time is spent and print [HT-PROF] reports every 5s.
-- Remove the file when done diagnosing -- it adds a small amount of wrapping
-- overhead.
do
    local ok, prof = pcall(require, 'heal_tracker_profile')
    if ok then _G._ht_profile = prof end
end
...
-- TLO call entirely.
-- Cache for isPlayerInZone results. Without this, every damage line for
-- a mob causes a full mq.TLO.Spawn lookup (synchronous game-state read,
-- typically 1-10ms per call). On a 54-man raid that's the dominant cost.
-- We remember both positives (it IS a player) and negatives (it ISN'T)
-- with a TTL so a player joining/leaving the zone is picked up within
-- a few seconds.
_G._HT_PlayerLookupCache = _G._HT_PlayerLookupCache or { hit = {}, miss = {}, expiry = 0 }

local function isPlayerInZone(name)
    if type(name) ~= 'string' or name == '' then return false end
    if knownChars[name] then return true end

    local cache = _G._HT_PlayerLookupCache
    local nowMsLocal = (mq and mq.gettime and mq.gettime()) or (os.time() * 1000)

    -- Periodic cache sweep -- discard everything every 10s. This is cheaper
    -- than per-entry TTL tracking and gives a stable upper bound on staleness.
    if nowMsLocal >= cache.expiry then
        cache.hit  = {}
        cache.miss = {}
        cache.expiry = nowMsLocal + 10000
    end

    if cache.hit[name]  then return true  end
    if cache.miss[name] then return false end
...
_G.HT_TargetLooksLikeNpc = function(target)
    ...
    -- Cache to avoid 3x mq.TLO.Spawn lookups per damage line. Each Spawn
    -- call is a synchronous game-state read (1-10 ms). Three of them per
    -- damage event is 3-30 ms, which is the dominant per-line cost
    -- according to [HT-PROF] measurements. TTL sweep every 10s catches
    -- mobs spawning/despawning.
    _G._HT_NpcLookupCache = _G._HT_NpcLookupCache or { hit = {}, miss = {}, expiry = 0 }
    local cache = _G._HT_NpcLookupCache
    local nowMsLocal = (mq and mq.gettime and mq.gettime()) or (os.time() * 1000)
    if nowMsLocal >= cache.expiry then
        cache.hit  = {}
        cache.miss = {}
    ...
    if cache.hit[t]  then return true  end
    if cache.miss[t] then return false end
    ...
    local result = ok and found == true
    if result then
        cache.hit[t] = true
    else
        cache.miss[t] = true
...
local function recordDamage(rawAttacker, target, amount, dmgType)
    ...
    -- Post-kill duplicate guard.
    -- When MQ2HealParse closes a mob immediately on the slain line, EQ can still
    -- deliver one or two late damage lines for that same displayed mob name in
    -- the same second. Without this guard, those late lines create a brand-new
    -- active scope that then saves a second duplicate fight by timeout.
    -- Do not add top-level locals here; this file is already near Lua's
    -- 200-local main-chunk limit.
    if lastKillName and target == lastKillName and killGraceUntil and nowMs() < killGraceUntil then
        if config.debug and debugLog then
            debugLog(string.format('SKIP: post-kill late damage ignored (%s)', tostring(target)))
        end
        return
    end
...
-- =============================================================================
-- MQ2HealParse plugin bridge install
-- =============================================================================
-- All hooks (recordHeal, recordDamage, recordSpellCast, onKill, isDriver)
-- are defined by this point in the file, so this is the safe place to wire
-- the plugin -> Lua bridge.
--
-- If MQ2HealParse plugin isn't loaded, the bridge prints a warning and
-- falls back to the Lua log tailer. If the plugin IS loaded, the bridge
-- subscribes to its /hpevt|... events and disables the Lua tailer to
-- avoid double-counting.
do
    local ok, bridge = pcall(require, 'heal_tracker_bridge')
    if ok and bridge and bridge.install then
...
local function processCombatLine(line)
    ...
    -- These lines usually do not contain damage numbers, so capture them before
    -- the damage-only gate below.
    if _G.HT_RecordObservedBurnLine and _G.HT_RecordObservedBurnLine(line, activeMobs, config) then
        return true
    end
...
-- =============================================================================
-- Engage the fast parser if heal_tracker_fastparse.lua loaded successfully.
-- The original processCombatLine above stays as the fallback when the module
-- is missing.
-- =============================================================================
if _G._ht_fastparse and _G._ht_fastparse.makeProcessCombatLine then
    processCombatLine = _G._ht_fastparse.makeProcessCombatLine({
    ...
    print('\ag[HealTracker]\ax fast parser engaged (processCombatLine)')
end

-- Profiler wrap: measures total wall time inside processCombatLine and
-- recordDamage so the [HT-PROF] report can tell us which is the bottleneck.
if _G._ht_profile then
    processCombatLine = _G._ht_profile.wrap('processCombatLine', processCombatLine)
    recordDamage      = _G._ht_profile.wrap('recordDamage',      recordDamage)
    print('\at[HT-PROF]\ax profiler attached')
end
...
local function logTailerPoll()
    ...
    while true do
        ...
        count = count + 1
        if count > (config.fastDpsMode and 150000 or 50000) then break end
...
-- =============================================================================
-- Engage the fast log tailer if available. Same fallback rule: if the module
-- failed to load, the original logTailerPoll above keeps running unchanged.
-- =============================================================================
if _G._ht_fastparse and _G._ht_fastparse.makeLogTailerPoll then
    ...
    print('\ag[HealTracker]\ax fast parser engaged (logTailerPoll)')
end
...
-- Plugin mode still needs these local heal mq.event listeners.
-- MQ2HealParse can drive DPS/kills/spells, but each EQ client only sees
-- some received-heal text locally. These events catch local heals/runes and
-- Actor-broadcast them to the driver without re-enabling DPS log parsing.
local function bindLocalHealEvents()
...
local function slashCmd(...)
    ...
    -- Allow /healtracker start to wake the script from the crash-safe pause.
    -- All other commands stay ignored while stopped so queued events/UI work
    -- cannot re-enter parser code during a stop.
    if shuttingDown and cmd ~= 'start' and cmd ~= 'resume' then return end
...
local btnVariants = {
    ...
    -- Per-tab active colors. Each tab gets a distinct identity color when
    -- selected so users can tell tabs apart at a glance without needing
    ...
    tab_settings = { {110/255, 125/255, 145/255, 1}, {150/255, 165/255, 185/255, 1}, {240/255, 245/255, 250/255, 1} }, -- slate

    -- Inactive (unselected) tab variants -- same hue family, but darker and
    -- desaturated. This way tabs keep their color identity at all times
...
local function drawLastFightWindow_impl()
    ...
    pcall(function()
        -- Tiny logo in the upper-left of the borderless Last Fight popup.
        -- This replaces the hidden title bar with a branded header row.
        if _G.HT_LOGO_TINY then
            HT_DrawLogoImage(_G.HT_LOGO_TINY, 48, 48)
            ImGui.SameLine(0, 8)
            ImGui.TextColored(_G.HT_MiniGold[1], _G.HT_MiniGold[2], _G.HT_MiniGold[3], 1.0, 'Last Fight Summary')
            ImGui.Separator()
        end
...
local function drawMini()
    ...
    pcall(function()
    if shouldDraw then
        -- Small logo/icon in the minimized floating tracker.
        if _G.HT_LOGO_MINI then
            HT_DrawLogoImage(_G.HT_LOGO_MINI, 64, 64)
            ImGui.SameLine(0, 8)
        end
        ...
        if showDps then
            ...
            -- GamParse-style live display snapshot. The parser still updates
            -- the real live scope continuously, but the mini DPS UI only
            -- copies live totals into this frozen display cache periodically.
            --
            -- v3.21.2 (fastparse) changes:
            --   1. Refresh every ~250ms instead of 1s. GamParse actually
            --      updates 3-4x per second; the 1s freeze made HealTracker
            --      feel laggy by comparison.
            --   2. Shallow clone of stats (just total/count/max + pet flag).
            --      The mini bar doesn't read targets/pets/dmgTypes -- the
            --      old deep clone wasted ~5-20ms per second on a 54-attacker
            --      raid scope, and the parser was paused for that window.
            _G.HT_LiveDpsDisplayCache = _G.HT_LiveDpsDisplayCache or nil
            _G.HT_LiveDpsDisplayTickMs = _G.HT_LiveDpsDisplayTickMs or 0
            local nowMsTick = (mq and mq.gettime and mq.gettime()) or (os.time() * 1000)
            -- Display refresh interval in ms. 250 = snappy GamParse feel.
            -- Configurable: set config.liveDpsRefreshMs to override.
            local refreshMs = tonumber(config.liveDpsRefreshMs) or 250
            if refreshMs < 100 then refreshMs = 100 end
            local liveCount = liveScope and (liveScope.count or 0) or 0
            if liveCount <= 0 then
                _G.HT_LiveDpsDisplayCache = liveScope
                _G.HT_LiveDpsDisplayTickMs = nowMsTick
            elseif (not _G.HT_LiveDpsDisplayCache)
                   or (nowMsTick - _G.HT_LiveDpsDisplayTickMs) >= refreshMs then
                local frozen = emptyDamageScope(liveScope.label or 'Active')
                frozen.total = liveScope.total or 0
                frozen.count = liveScope.count or 0
                frozen.max = liveScope.max or 0
                frozen.started = liveScope.started
                frozen.ended = liveScope.ended
                frozen.mobLevel = liveScope.mobLevel
                frozen.stats = {}
                -- Shallow snapshot. Mini bar only needs total/count/max and
                -- knowledge that pets exist for the "+ pets" label. The
                -- full DPS tab reads liveScope directly (not frozen), so it
                -- still gets the full targets/pets/dmgTypes breakdown.
                for atk, st in pairs(liveScope.stats or {}) do
                    local petsRef = st.pets
                    local hasPets = false
                    if petsRef then
                        for _k in pairs(petsRef) do hasPets = true; break end
                    end
                    frozen.stats[atk] = {
                        total    = st.total or 0,
                        count    = st.count or 0,
                        max      = st.max or 0,
                        -- Share refs (don't deep-copy). The mini bar reads
                        -- these only when the user opens the full breakdown,
                        -- and by then the next refresh tick will have happened.
                        -- Race risk is cosmetic only (a row might briefly
                        -- show stale subdata); no parse correctness impact.
                        targets  = petsRef and st.targets  or nil,
                        pets     = petsRef,
                        dmgTypes = st.dmgTypes,
                    }
                end
                _G.HT_LiveDpsDisplayCache = frozen
                _G.HT_LiveDpsDisplayTickMs = nowMsTick
            end

            -- Mini bar = frozen 1-second DPS display cache. Completed
            -- fights are shown in a SEPARATE window (drawLastFightWindow).
            local displayScope = _G.HT_LiveDpsDisplayCache or liveScope
...
_G.HT_DrawDamageTypeBreakdown = _G.HT_DrawDamageTypeBreakdown or function(scope, idPrefix, durationSec, spellsScope)
    ...
    for attacker, st in pairs(scope.stats or {}) do
        ...
        -- Always include damage rows so History fights saved before the
        -- damage-type upgrade can still use Player Compare. Older archive
        -- entries may not have dmgTypes yet, but they still have total/hits/
        -- max/DPS data and spell-cast comparison can still work.
        if total > 0 then
...
_G.HT_NEW_TRIGGER = _G.HT_NEW_TRIGGER or {
    pattern      = '',
    label        = '',
    color        = 'red',
    beep         = true,
...
_G.HT_EDIT_TRIGGER_IDX = nil  -- index of trigger being edited (nil = none)
...
_G.HT_drawFull = function()
    ...
    pcall(function()
        if shouldDraw and config.windowOpen then
            -- Main window branded header: large centered HealTracker logo.
            -- Draw this before the Mini button/page tabs so it is obvious when loaded.
            if not _G.HT_LOGO_MAIN then HT_LoadLogoTextures(true) end
            if _G.HT_LOGO_MAIN then
                local logoW, logoH = 420, 140
                local winW = tonumber(ImGui.GetWindowWidth()) or 0
                if winW > logoW then
                    ImGui.SetCursorPosX((winW - logoW) * 0.5)
                end
                local drewLogo = HT_DrawLogoImage(_G.HT_LOGO_MAIN, logoW, logoH)
            ...
            -- active tab uses the bright "tab_<name>" variant; inactive
            -- tabs use the darker, desaturated "tab_<name>_off" variant
            -- so they keep their color identity but visibly recede.
            local TAB_VARIANTS = {
                ...
                settings = { on = 'tab_settings', off = 'tab_settings_off' },
            }
            ...
            local function pageButton(label, key)
                ...
                local shown
                if active then
                    -- A leading filled circle marks the active tab.
                    shown = '● ' .. label
                else
                    -- Same indent so labels line up between states.
                    shown = '  ' .. label
                end
                local variants = TAB_VARIANTS[key] or { on = 'primary', off = 'secondary' }
                local variant = active and variants.on or variants.off
                if btn(shown .. '##page_' .. key, variant, bw, bh) then
                    page = key
                    config.lastTab = key
                    saveConfig()
                end
            ...
            -- Single-row glossy page buttons. Each tab gets a distinct color
            -- identity (green/red/purple/amber/orange/slate) so a glance at
            -- the active tab tells you what page you're on without reading.
...
_G.HT_cleanup = function()
    -- v3.21.12: ultra-safe direct /lua stop guard.
    -- Direct /lua stop happens inside MQ2Lua's forced teardown path. On some
    -- MQ builds, calling mq.imgui.destroy(), mq.unbind(), print(), TLOs, or
    -- file IO from that path crashes in mq2lua.dll/vsprintf_s_l.
    -- So cleanup intentionally does NOT call any MQ APIs. It only flips Lua
    -- flags. MQ will reclaim the Lua state. Use /healtracker stop for a clean
    -- in-game pause; /lua stop should now avoid our script doing unsafe work.
    shuttingDown = true
...
_G.HT_boot = function()
    ...
    -- The bridge feeds recordHeal/recordDamage/etc. via the plugin's
    -- /hpevt|... events instead.
    local _plugin_active = false
    pcall(function()
        _plugin_active = mq.TLO.HealParse.Enabled() == true
    end)
...
while M.running do
    ...
    end -- legacy stop-request else
    end
end

pcall(function() if _G.HT_cleanup then _G.HT_cleanup() end end)
-- Do not close files or call MQ APIs during script teardown; direct /lua stop
-- on some MQ2Lua builds can crash if teardown code performs IO/API calls.
